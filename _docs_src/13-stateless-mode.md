...
4. Nothing else. Types keep compiling because `Sisp extends StatelessSisp`, the shared routes keep their paths and contracts, and `callback:*` listeners keep firing without an edit. The rejection reasons match too: `POST /callback` emits `callback_replayed` for a duplicate delivery and `unknown_transaction` for a reference nobody created in either mode, so a monitor counting rejections by reason reads the same after the switch. What you gain are the stateful-only events, `payment:*` and `transaction:*`.
