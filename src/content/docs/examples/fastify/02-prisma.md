---
title: "Fastify with Prisma storage"
description: "The same Fastify app as knex storage, but persistence runs on your own Prisma client. You inject the adapter through storage; the package core never imports..."
sidebar:
  order: 2
---

The same Fastify app as [knex storage](/examples/fastify/01-knex/), but persistence runs on your own Prisma client. You inject the adapter through `storage`; the package core never imports Prisma. Behavior matches the knex adapter (the shared storage contract suite runs against both).
...
The payment form, the auto-submitting gateway hop, the callback flow, and the live-gateway gotchas are identical to the [knex example](/examples/fastify/01-knex/) - only the `storage` wiring changes.
...
- Other ORMs (Drizzle, Sequelize, TypeORM) can implement the same `SispStorage` port. See [Storage adapters](/12-storage-adapters/).

**Next:** [Server-side rendering](/examples/fastify/03-ssr/)
