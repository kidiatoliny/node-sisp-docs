---
title: "Payment Flow"
description: "Both flows run through pipelines of small single-purpose pipes, the same semantics as the Laravel package."
sidebar:
  order: 4
---
...
For a SPA that talks to the backend with `fetch`, post to `/payment/intent` instead. It runs the same pipeline but returns the gateway target as JSON `{ action, fields, ref }`; the frontend then builds the form and submits it full-page. The HTML `/payment` route cannot be consumed over `fetch` (the returned markup will not navigate and its inline script will not run). See [Adapters](/06-adapters/#payment-versus-paymentintent).
...
**Next:** [Transaction Management](/05-transaction-management/)
